...
public enum GameTimer {
	...
	public void setup(long initialTimerValue) {
		...
		Log.d("GameTimer", "Timer setting up. initial value is " + this.initialTimerValue);
		this.timer = new CustomCountDownTimer(this.initialTimerValue, 1000) {
			
			@Override
			public void tick(long millisUntilFinished) {
				Log.d("GameTimer", "Timer value is   " + this.getMillisUntilFinished());
				Game.game.timerUpdated(millisUntilFinished);
			}

			@Override
			public void finish() {
				Log.d("GameTimer", "Timer stopped at " + this.getMillisUntilFinished());
	...
	public void start() {
		Log.d("GameTimer", "Timer starting. value is " + this.timer.getMillisUntilFinished());
		this.timer.start();
	}

	public void stop() {
		Log.d("GameTimer", "Timer stopping. value is " + this.timer.getMillisUntilFinished());
		this.timer.cancel();
	}
	
	public void pause() {
		this.timer.cancel();
		Log.d("GameTimer", "Timer pausing. value is " + this.timer.getMillisUntilFinished());
		this.setup(this.timer.getMillisUntilFinished());
	}
	
	public void resume() {
		Log.d("GameTimer", "Timer resuming. value is " + this.timer.getMillisUntilFinished());
		this.timer.start();
	}
